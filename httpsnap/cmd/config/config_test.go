...
func TestTLSPeerConfig(t *testing.T) {
	...
	if err != nil {
		t.Fatal("Not supposed to get error while getting client cert when 'tls.usePeerConfig' enabled and 'tls.clientCert' is missing")
	}
	if clientCert == "" {
	...
	if err != nil {
		t.Fatal("Not supposed to get error while getting ca certs when 'tls.usePeerConfig' enabled and 'tls.caCerts' is missing")
	}
	if len(caCerts) == 0 {
