...
// GetNamedClientOverridePath returns map of ClientCert
func (c *config) GetNamedClientOverride() (clientCert map[string]*httpsnapApi.ClientCrt, err error) {
	var clientCrt map[string]*httpsnapApi.ClientCrt
	err = c.httpSnapConfig.UnmarshalKey("tls.namedClientOverride", &clientCrt)
	if err != nil {
		return nil, err
	}
