...
func (httpServiceImpl *HTTPServiceImpl) NewInvoker(httpServiceInvokeRequest HTTPServiceInvokeRequest) (Invoker, errors.Error) {
	...
	// Validate URL
	_, err := httpServiceImpl.validateURL(httpServiceInvokeRequest)
	if err != nil {
		return nil, errors.WithMessage(errors.ValidationError, err, "Failed URL Validation")
	}
...
func (httpServiceImpl *HTTPServiceImpl) getTLSConfig(client string, config httpsnapApi.Config) (*tls.Config, errors.Error) {
	...
	if err != nil {
		return nil, errors.Errorf(errors.MissingConfigDataError, "client crt not found: '%s'", err)
	}
