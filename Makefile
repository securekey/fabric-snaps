...
FABRIC_NEXT_PEER_IMAGE = securekey/fabric-peer
FABRIC_NEXT_IMAGE_TAG = 1.1.0-0.0.1-snapshot-75ec399

DOCKER_OUTPUT_NS ?= repo.onetap.ca:8443/next/hyperledger/fabric-snaps
...
docker: all
	@docker build -f ./images/fabric-snaps/Dockerfile --no-cache -t $(DOCKER_OUTPUT_NS):$(ARCH)-$(PROJECT_VERSION) \
