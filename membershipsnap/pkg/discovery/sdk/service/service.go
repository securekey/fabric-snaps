...
func (s *MembershipService) invokeMembershipSnap() ([]sdkapi.Peer, error) {
	...
	for _, p := range npeers {
		org, err := s.getOrgNameFromMSP(p.MspID)
		logger.Debugf("org '%s' found for mspID '%s' \n", p.MspID)
		if err != nil {
			nbErrs++
			errArr = append(errArr, err)
			logger.Warnf("org not found for MspID '%s'. Error: %s \n", p.MspID, err)
			continue
		}
		pName, err := s.getPeerNameFromChannelPeer(s.channelID, p)
		logger.Debugf("peerName '%s' \n", pName)
		if err != nil {
			nbErrs++
			errArr = append(errArr, err)
			logger.Warnf("getting peerName returned error. Error: %s \n", err)
			continue
		}
		pc, err := s.clientConfig.PeerConfig(org, pName)
		logger.Debugf("peerConfig '%+v' found for org '%s' and peerName '%s' \n", pc, org, pName)
		if err != nil {
			nbErrs++
			errArr = append(errArr, err)
			logger.Warnf("peerConfig not found for org '%s' and MspID '%s'. Error: %s \n", org, p.MspID, err)
			continue
		}
		sdkPeer, err := sdk.FabricProvider().CreatePeerFromConfig(&apiconfig.NetworkPeer{PeerConfig: *pc, MspID: p.MspID})
...
// TODO: possibly move this function to GO SDK's Config api and expose it
func (s *MembershipService) getOrgNameFromMSP(mspID string) (string, error) {
	config, err := s.clientConfig.NetworkConfig()
	if err != nil {
		return "", err
	}
	for k, org := range config.Organizations {
		logger.Debugf("key '%s' org '%s' \n", k, org)
		if strings.EqualFold(org.MspID, mspID) {
			return k, nil
		}
	}
	return "", fmt.Errorf("Org not found in the client config for MSP: %s", mspID)
}

func (s *MembershipService) getPeerNameFromChannelPeer(channelID string, cp apiconfig.ChannelPeer) (string, error) {
	c, err := s.clientConfig.NetworkConfig()
	if err != nil {
		return "", err
	}
	ch, ok := c.Channels[channelID]
	if !ok {
		return "", fmt.Errorf("could not get channel object from config with channelID: '%s'", channelID)
	}
	for k, v := range ch.Peers {
		if v == cp.PeerChannelConfig {
			return k, nil
		}
	}
	return "", fmt.Errorf("peerChannelConfig not found channelID: '%s' and channelPeer.MspID: %s", channelID, cp.MspID)
}
