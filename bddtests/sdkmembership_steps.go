...
func (t *MembershipSDKSteps) injectMembershipProviderIntoSDK() error {
	...
	// Create SDK setup for channel client with membership discovery service
	// This step is performed during the test to allow normal SDK-based initialized of the membership discovery provider
	_, err := fabsdk.New(
		fabsdk.WithConfig(t.BDDContext.ClientConfig()),
		fabsdk.WithServicePkg(&factory))
	if err != nil {
		return errors.WithMessage(err, "Failed to create new SDK with membership discovery provider included")
	}
	return nil
}
...
// CreateSelectionProvider returns a new implementation of membership discovery provider
