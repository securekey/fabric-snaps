...
func (d *CommonSteps) joinPeersToChannel(channelID, orgID string, peersConfig []coreApi.PeerConfig) error {
	...
	peer, err := d.BDDContext.sdk.FabricProvider().CreatePeerFromConfig(&coreApi.NetworkPeer{PeerConfig: peersConfig[0]})
	if err != nil {
	...
	alreadyJoined, err := HasPrimaryPeerJoinedChannel(channelID, resourceMgmt, d.BDDContext.OrgUser(orgID, ADMIN), peer)
	if err != nil {
	...
	chMgmtClient, err := d.BDDContext.OrgClient(orgID, ADMIN).ResourceMgmt()
	if err != nil {
...
func (d *CommonSteps) InvokeCCWithArgs(ccID, channelID string, targets []*PeerConfig, args []string, transientData map[string][]byte) error {
	...
	for _, target := range targets {
		targetPeer, err := d.BDDContext.sdk.FabricProvider().CreatePeerFromConfig(&coreApi.NetworkPeer{PeerConfig: target.Config})
		if err != nil {
			return errors.WithMessage(err, "NewPeer failed")
		}
...
func (d *CommonSteps) instantiateChaincodeWithOpts(ccType, ccID, ccPath, orgIDs, channelID, args, ccPolicy, collectionNames string, allPeers bool) error {
	...
	for _, pconfig := range peers {
		...
		sdkPeer, err := d.BDDContext.sdk.FabricProvider().CreatePeerFromConfig(&coreApi.NetworkPeer{PeerConfig: pconfig.Config})
		if err != nil {
			return errors.WithMessage(err, "NewPeer failed")
		}
