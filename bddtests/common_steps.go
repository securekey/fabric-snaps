...
func (d *CommonSteps) queryCC(ccID, args, orgID string) error {
	...
	queryResult, err := d.QueryCCWithArgs(ccID, "", strings.Split(args, ","), []*PeerConfig{&PeerConfig{Config: peersConfig[0], OrgID: orgID, MspID: d.BDDContext.peersMspID[serverHostOverride], PeerID: serverHostOverride}}...)
	if err != nil {
		return fmt.Errorf("QueryCCWithArgs return error: %v", err)
	}
	queryValue = queryResult
...
func (d *CommonSteps) RegisterSteps(s *godog.Suite) {
	...
	s.Step(`^client queries chaincode "([^"]*)" with args "([^"]*)" on org "([^"]*)"$`, d.queryCC)
