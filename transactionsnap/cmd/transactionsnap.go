...
var txSnapService *txsnapservice.TxServiceImpl
...
func getChannelID(stub shim.ChaincodeStubInterface) (string, error) {
	var channel string
	function, stringargs := stub.GetFunctionAndParameters()
	args := stub.GetArgs()
	switch function {
	case "endorseTransaction":
		if len(args) < 2 {
			return "", errors.Errorf("ChaincodeID is mandatory field of the SnapTransactionRequest")
		}
		...
		if snapTxRequest.ChannelID == "" {
			return "", errors.Errorf("ChannelID is mandatory field of the SnapTransactionRequest")
		}
		channel = snapTxRequest.ChannelID
	case "commitTransaction":
		if len(args) < 4 {
		...
		if channel == "" {
			return "", errors.Errorf("Cannot create channel Error creating new channel: name is required")
		}
	case "endorseAndCommitTransaction":
		if len(args) < 2 {
	...
	case "verifyTransactionProposalSignature":
		if len(stringargs) == 0 {
			return "", errors.New("Expected args containing channelID()")
		}
		if string(args[0]) == "" {
			return "", errors.New("Expected args containing channelID(empty)")
		}
		channel = string(args[1])
	...
	case "getPeersOfChannel":
		...
		if len(args) < 1 {
			return "", errors.Errorf("Channel name must be provided")
		}
...
func configureTxService(channelID string) error {
	a, err := txsnapservice.Get(channelID)
	if err != nil {
		return err
	}
	txSnapService = a
...
func getPeersOfChannel(args []string) ([]byte, error) {
	...
	channelMembership := txSnapService.Membership.GetPeersOfChannel(channel, true)
...
func endorseTransaction(args [][]byte) ([]*apitxn.TransactionProposalResponse, error) {
	...
	tpxResponse, err := txSnapService.EndorseTransaction(snapTxRequest, nil)
	if err != nil {
		return nil, err
	}
...
func commitTransaction(args [][]byte, timeout time.Duration) (pb.TxValidationCode, error) {
	...
	validationCode, err := txSnapService.CommitTransaction(channelID, tpResponses, registerTxEvent, registerTxEventTimeout)
	if err != nil {
...
func verifyTxnProposalSignature(args [][]byte) error {
	...
	err := txSnapService.VerifyTxnProposalSignature(channelID, signedProposal)
	if err != nil {
