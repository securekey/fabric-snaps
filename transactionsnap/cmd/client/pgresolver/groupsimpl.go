	...
	if index >= len(grps) {
		...
		for _, c := range o.stack.Groups() {
			...
			if c.index >= len(groupItems) {
				...
				logger.Errorf("***** Stack index %d is greater than or equal to number of items: %d. This is a problem that should be looked into!!! groupItems=%v, index=%d", c.index, len(groupItems), groupItems, index)
			} else {
