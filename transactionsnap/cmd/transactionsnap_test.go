...
func TestMain(m *testing.M) {
	...
	fcClient = txSnapService.FcClient

	newtworkConfig, _ := txSnapService.FcClient.GetConfig().NetworkConfig()
	newtworkConfig.Orderers["orderer.example.com"] = apiconfig.OrdererConfig{URL: broadcastTestURL}

	//create selection service
	peer, _ := sdkFabApi.NewPeer(endorserTestURL, "", "", txSnapService.FcClient.GetConfig())
	selectionService := mocks.MockSelectionService{TestEndorsers: []sdkApi.Peer{peer},
		TestPeer:       api.PeerConfig{EventHost: endorserTestEventHost, EventPort: endorserTestEventPort},
		InvalidChannel: ""}

	txSnapService.FcClient.SetSelectionService(&selectionService)
	...
	testChannel, err := txSnapService.FcClient.NewChannel("testChannel")
	if err != nil {
