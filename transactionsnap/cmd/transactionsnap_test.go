...
func TestNotSpecifiedChannel(t *testing.T) {
	...
	for _, value := range funcs {
		...
		if value == "verifyTransactionProposalSignature" {
			...
			args = createTransactionSnapRequest(value, "ccid", "")
		}
		//invoke transaction snap
		response := stub.MockInvoke("TxID", args)
...
func TestTransactionSnapInvokeFuncCommitTransactionSuccess(t *testing.T) {
	...
	stub := shim.NewMockStub("transactionsnap", snap)
	//invoke transaction snap
	args := createTransactionSnapRequest("commitTransaction", "ccid", "testChannel")
	response := stub.MockInvoke("TxID2", args)
	if response.Status != shim.OK {
...
func TestMain(m *testing.M) {
	...
	configData, err := ioutil.ReadFile("../pkg/mocks/config/config.yaml")
	if err != nil {
	...
	_, err = config.NewConfig("../pkg/mocks/config", channelID)
	if err != nil {
		panic(fmt.Sprintf("Error initializing config: %s", err))
	}

	txsnapservice.PeerConfigPath = sampleconfig.ResolvPeerConfig("../pkg/mocks/config")

	txSnapConfig, err = config.NewConfig("../pkg/mocks/config", channelID)
	if err != nil {
		panic(fmt.Sprintf("Error initializing config: %s", err))
	}
