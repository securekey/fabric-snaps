...
func (f *peerFilter) Accept(p fabApi.Peer) bool {
	...
	if !accepted {
		logger.Debugf("Peer [%s] will NOT be accepted since its block height for channel [%s] is %d which is less than or equal to that of the required minimum: %d.", p.URL(), f.channelID, peerHeight, f.height)
	} else {
