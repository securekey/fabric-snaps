...
func (c *clientImpl) initialize(channelID string, serviceProviderFactory apisdk.ServiceProviderFactory) error {
	...
	// Create local peer
	peer, err := sdkpeer.New(clientConfig, sdkpeer.WithTLSCert(c.txnSnapConfig.GetTLSCert()),
		sdkpeer.WithURL(fmt.Sprintf("%s:%d", localPeer.Host, localPeer.Port)))
	if err != nil {
		return errors.WithMessage(errors.GeneralError, err, "Error creating new peer")
	}
	peer.SetMSPID(string(localPeer.MSPid))
	...
	sdk, err := fabsdk.New(fabsdk.WithConfig(clientConfig),
		fabsdk.WithContextPkg(&factories.CredentialManagerProviderFactory{CryptoPath: c.txnSnapConfig.GetMspConfigPath()}),
		fabsdk.WithCorePkg(&factories.DefaultCryptoSuiteProviderFactory{ProviderName: cryptoProvider, Peer: peer}),
