...
func (c *clientImpl) endorseTransaction(endorseRequest *api.EndorseTxRequest, options ...endorse.RequestOption) (*channel.Response, errors.Error) {
	...
	if err != nil {
		return nil, errors.WithMessage(errors.SystemError, err, "Failed to read opts")
	}
...
func (c *clientImpl) commitOnlyTransaction(rwSetIgnoreNameSpace []api.Namespace, commitType api.CommitType, endorserResponse *channel.Response, registerTxEvent bool, callback api.EndorsedCallback) (*channel.Response, bool, errors.Error) {
	...
	customExecuteHandler := handler.NewPreEndorsedHandler(endorserResponse, checkForCommit)
	...
	resp, err := c.invokeHandler(
		customExecuteHandler,
		// put dummy values for ChaincodeID and fcn because sdk require them even if not used in handlersgi
