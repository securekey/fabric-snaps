...
func (f *DefaultCryptoSuiteProviderFactory) NewFabricProvider(context fab.ProviderContext) (apisdk.FabricProvider, error) {
	return &CustomFabricProvider{FabricProvider: fabpvdr.New(context), providerContext: context, peer: f.Peer}, nil
...
func (f *CustomFabricProvider) CreateChannelConfig(ic fab.IdentityContext, channelID string) (fab.ChannelConfig, error) {
	...
	return chconfig.New(ctx, channelID, chconfig.WithPeers([]fab.Peer{f.peer}))
}
