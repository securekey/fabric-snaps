...
func (txs *TxServiceImpl) EndorseTransaction(snapTxRequest *api.SnapTransactionRequest, peers []sdkApi.Peer) ([]byte, error) {
...
func (txs *TxServiceImpl) CommitTransaction(snapTxRequest *api.SnapTransactionRequest, peers []sdkApi.Peer) error {
	request, err := txs.createEndorseTxRequest(snapTxRequest, peers)
	if err != nil {
		return err
	}
	err = txs.FcClient.CommitTransaction(request, snapTxRequest.TimeoutTxEvent, txs.Callback)
	if err != nil {
		return errors.WithMessage(errors.GeneralError, err, "Received error when endorsing Tx")
	}
