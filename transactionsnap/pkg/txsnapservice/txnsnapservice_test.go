...
func TestEndorseTransaction(t *testing.T) {
	...
	value, err := txService.EndorseTransaction(&snapTxReq, nil)
	if err != nil {
		t.Fatalf("Error endorsing transaction %v", err)
	}
	if string(value) != "value" {
		t.Fatalf("Expected to receive value but got %s", string(value))
	}
...
func TestCommitTransaction(t *testing.T) {
	...
	err := txService.CommitTransaction(&snapTxReq, nil)
	if err != nil {
