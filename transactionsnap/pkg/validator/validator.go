...
func newValidator(channelID string, qeProvider queryExecutorProvider, pe validation.PolicyEvaluator, des msp.IdentityDeserializer) *validator {
	v := &validator{
		...
		policyEvaluator:       pe,
		idDeserializer:        des,
		ccDataCache: lazycache.New("chaincode-data",
			func(key lazycache.Key) (interface{}, error) {
				return createCCDefinition(key.String(), qeProvider)
			},
		),
	}
...
func (v *validator) ValidateProposalResponses(proposal *pb.SignedProposal, proposalResponses []*pb.ProposalResponse) (pb.TxValidationCode, error) {
	...
	for i, pr := range proposalResponses {
		if pr.Endorsement == nil {
			return pb.TxValidationCode_INVALID_OTHER_REASON, errors.New("no endorsement in proposal response")
		}
...
func (v *validator) validateEndorsements(proposal *pb.SignedProposal, prespBytes []byte, endorsements []*pb.Endorsement) (pb.TxValidationCode, error) {
	...
	if code, err := validateChaincodeAction(proposal, chaincodeAction); err != nil {
		return code, err
	}
...
func (v *validator) validateEndorsementPolicy(ccID string, chaincodeAction *pb.ChaincodeAction, prespBytes []byte, endorsements []*pb.Endorsement) (pb.TxValidationCode, error) {
	...
	txcc, policy, err := v.getCCInfo(ccID)
	if err != nil {
		return pb.TxValidationCode_INVALID_OTHER_REASON, err
	}
	...
	if ccID == chaincodeAction.ChaincodeId.Name && txcc.ChaincodeVersion != chaincodeAction.ChaincodeId.Version {
		return pb.TxValidationCode_EXPIRED_CHAINCODE, errors.Errorf("chaincode %s:%s/%s didn't match %s:%s/%s in lscc", ccID, chaincodeAction.ChaincodeId.Version, v.channelID, txcc.ChaincodeName, txcc.ChaincodeVersion, v.channelID)
	}

	if err := v.validatePolicyForCC(prespBytes, endorsements, policy); err != nil {
		switch err.(type) {
...
func (v *validator) getCCInfo(ccID string) (*sysccprovider.ChaincodeInstance, []byte, error) {
	...
	cd, err := v.getCCDefinition(ccID)
	if err != nil {
		logger.Errorf("Unable to get chaincode data from ledger due to %s", err)
		return nil, nil, err
	}
	...
	var policy []byte
	_, policy = cd.Validation()

	return cc, policy, nil
}

func (v *validator) getCCDefinition(ccid string) (ccprovider.ChaincodeDefinition, error) {
	ccData, err := v.ccDataCache.Get(lazycache.NewStringKey(ccid))
	if err != nil {
		return nil, err
	}
	return ccData.(ccprovider.ChaincodeDefinition), nil
}
...
func (v *validator) validateResponse(response *pb.ProposalResponse) error {
	...
	if response.Response.Status < int32(common.Status_SUCCESS) || response.Response.Status >= int32(common.Status_BAD_REQUEST) {
...
func deduplicateIdentity(prespBytes []byte, endorsements []*pb.Endorsement) ([]*common.SignedData, error) {
...
func createCCDefinition(ccid string, qeProvider queryExecutorProvider) (ccprovider.ChaincodeDefinition, error) {
