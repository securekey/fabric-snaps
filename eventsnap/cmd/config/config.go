...
func getTLSCredentials(peerConfig, config *viper.Viper) (credentials.TransportCredentials, error) {
	...
	if config.GetBool("eventsnap.relay.tlsCerts.systemCertPool") == true {
		var err error
	...
	logger.Debugf("tls client cert: %s", peerConfig.GetString("eventsnap.relay.tlsCerts.client.certfile"))
	logger.Debugf("tls client key: %s", peerConfig.GetString("eventsnap.relay.tlsCerts.client.keyfile"))

	var certificates []tls.Certificate
	if config.GetString("eventsnap.relay.tlsCerts.client.certfile") != "" {
		clientCerts, err := tls.LoadX509KeyPair(config.GetString("eventsnap.relay.tlsCerts.client.certfile"), config.GetString("eventsnap.relay.tlsCerts.client.keyfile"))
		if err != nil {
