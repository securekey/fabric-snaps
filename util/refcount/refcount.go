...
func (c *ReferenceCounter) Acquire() bool {
	if c.isClosed() {
		logger.Infof("Cannot acquire reference since resource is already closed")
		return false
	}
...
func (c *ReferenceCounter) Close() bool {
	if !c.close() {
		logger.Infof("Resource already closed.")
		return false
	}

	logger.Infof("Current refcount: %d. Resource will be closed when last reference is removed.", c.count())
	c.checkAndCloseResource()
...
func (c *ReferenceCounter) checkAndCloseResource() {
	if !c.isClosed() {
		logger.Debugf("Not closed")
		return
	}

	// If the ref-count reaches 0 then close the client
	if c.setCount(0, -1) {
		logger.Infof("Last reference removed - closing resource ...")
		c.closer()
		logger.Infof("... closed.")
		return
	}
