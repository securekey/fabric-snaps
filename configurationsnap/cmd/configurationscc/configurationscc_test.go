...
func TestSetRefreshInterval(t *testing.T) {
	assert.Equal(t, uint32(5), atomic.LoadUint32(&refreshInterval), "Expected cache interval of 5")
	err := setRefreshInterval(getBytes("a", []string{"b"}))
	assert.NotNil(t, err, "cache refresh interval was not set properly")
	assert.Equal(t, uint32(5), atomic.LoadUint32(&refreshInterval), "Expected cache interval of 5")

	err = setRefreshInterval(getBytes("15", []string{"b"}))
	assert.Nil(t, err, "expected error")
	assert.Equal(t, uint32(15), atomic.LoadUint32(&refreshInterval), "Expected cache interval of 15")

	err = setRefreshInterval(getBytes("-1", []string{"b"}))
	assert.NotNil(t, err, "expected error 'cache refresh interval cannot be less then zero")
...
func TestTicker(t *testing.T) {
	...
	fmt.Printf("init")
	arg := []byte("2")
	res := stub.MockInit("txID", [][]byte{arg})
	if res.Status != shim.OK {
		t.Fatalf("Init failed: %v", res.Message)
	}
	time.Sleep(5 * time.Second)
	assert.Equal(t, uint32(2), atomic.LoadUint32(&refreshInterval), "Expected cache interval of 2 seconds")
	...
	time.Sleep(5 * time.Second)
	assert.Equal(t, uint32(1), atomic.LoadUint32(&refreshInterval), "Expected cache interval of 1 second")
