...
var supportedAlgs = []string{"ECDSA", "ECDSAP256", "ECDSAP384", "RSA", "RSA1024", "RSA2048", "RSA3072", "RSA4096", "AES", "AES128", "AES192", "AES256"}
...
func getKeyOpts(keyType string, ephemeral bool) (bccsp.KeyGenOpts, error) {
	...
	case "AES":
		return &bccsp.AESKeyGenOpts{Temporary: ephemeral}, nil
	case "AES128":
		return &bccsp.AES128KeyGenOpts{Temporary: ephemeral}, nil
	case "AES192":
		return &bccsp.AES192KeyGenOpts{Temporary: ephemeral}, nil
	case "AES256":
		return &bccsp.AES256KeyGenOpts{Temporary: ephemeral}, nil
	default:
...
func parseKey(k bccsp.Key) pb.Response {
	...
	if !k.Private() {
		...
		if err != nil {
			return shim.Error(fmt.Sprintf("Error:getting bytes %v", err))
		}
		logger.Debugf("***KeyBytes from public: %s", string(keyBytes))
		return shim.Success(keyBytes)
	}
	//get public key
