...
//Snaps array of snap configs.Each config represents snapmetadata defined either in config file or at runtime
...
func Init(configPathOverride string) error {
	...
	viper.AutomaticEnv()
	viper.SetEnvKeyReplacer(replacer)GetEndorsersForChaincode
...
func GetSnapConfig(snapName string) *SnapConfig {
	...
	for _, registeredSnap := range Snaps {
		if registeredSnap.Name == snapName {GetEndorsersForChaincode
