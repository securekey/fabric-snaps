...
func (cmngr *configManagerImpl) Save(configData []byte) errors.Error {
	...
	err1 := cmngr.stub.SetEvent(cfgsnapapi.ConfigCCEventName, nil)
	if err1 != nil {
		return errors.Wrap(errors.SystemError, err1, "SetEvent failed")
	}
...
func (cmngr *configManagerImpl) search(key api.ConfigKey) ([]*api.ConfigKV, errors.Error) {
	...
	if err != nil {
		return nil, errors.Wrap(errors.GetKeyError, err, "Failed to search Index and Fields for key")
	}
	configsMap, err := cmngr.getConfigurations(index, fields)
	if err != nil {
		return nil, errors.Wrap(errors.GetKeyError, err, "Failed to get configuration for s key")
	}
