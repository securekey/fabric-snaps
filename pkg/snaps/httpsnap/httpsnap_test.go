...
func TestNamedClient(t *testing.T) {
	...
	args := [][]byte{[]byte("https://localhost:8443/hello"), []byte(contentType), []byte(jsonStr), []byte("xyz")}
	...
	args = [][]byte{[]byte("https://localhost:8443/hello"), []byte(contentType), []byte(jsonStr), []byte("abc")}
	verifySuccess(t, stub, args, "Hello")

	// Happy path: Should get "Hello" back - nil named client is using default TLS settings
	args = [][]byte{[]byte("https://localhost:8443/hello"), []byte(contentType), []byte(jsonStr), nil}
	verifySuccess(t, stub, args, "Hello")

	// Happy path: Should get "Hello" back - empty named client is using default TLS settings
	args = [][]byte{[]byte("https://localhost:8443/hello"), []byte(contentType), []byte(jsonStr), []byte("")}
	verifySuccess(t, stub, args, "Hello")
...
func TestCertPinning(t *testing.T) {
	...
	args := [][]byte{[]byte("https://localhost:8443/hello"), []byte(contentType), []byte(jsonStr), []byte(""), []byte("c2MiEtoRw7m1kc2r4GnVCT89OxqXK24PFiK02Qo1PIs=")}
	verifySuccess(t, stub, args, "Hello")

	// Happy path: Should get "Hello" back - pinset is provided (comma separated)
	args = [][]byte{[]byte("https://localhost:8443/hello"), []byte(contentType), []byte(jsonStr), []byte(""), []byte("c2MiEtoRw7m1kc2r4GnVCT89OxqXK24PFiK02Qo1PIs=,pin2")}
	verifySuccess(t, stub, args, "Hello")

	// Happy path: Should get "Hello" back - nil pinset is provided (no cert pin validation)
	args = [][]byte{[]byte("https://localhost:8443/hello"), []byte(contentType), []byte(jsonStr), []byte(""), nil}
	verifySuccess(t, stub, args, "Hello")

	// Happy path: Should get "Hello" back - empty pinset is provided (no cert pin validation)
	args = [][]byte{[]byte("https://localhost:8443/hello"), []byte(contentType), []byte(jsonStr), []byte(""), []byte("")}
	verifySuccess(t, stub, args, "Hello")

	// Failed path: Invalid pinset is provided
	args = [][]byte{[]byte("https://localhost:8443/hello"), []byte(contentType), []byte(jsonStr), []byte(""), []byte("pin1,pin2,pin3")}
...
func TestJsonValidation(t *testing.T) {
	...
	args := [][]byte{[]byte("https://localhost:8443/hello"), []byte(contentType), []byte(jsonStr)}
	verifySuccess(t, stub, args, "Hello")

	// Failed path: Request fails schema validation
	invalidJSONStr := `{"test": "test"}`
	args = [][]byte{[]byte("https://localhost:8443/test/abc"), []byte(contentType), []byte(invalidJSONStr)}
	...
	args = [][]byte{[]byte("https://localhost:8443/test/invalidJSONResponse"), []byte(contentType), []byte(jsonStr)}
	...
	args = [][]byte{[]byte("https://localhost:8443/test/textResponse"), []byte(contentType), []byte(jsonStr)}
	...
	args = [][]byte{[]byte("https://localhost:8443/hello"), []byte("text/html"), []byte(jsonStr)}
...
func TestPost(t *testing.T) {
	...
	args = [][]byte{[]byte("https://www.google.ca"), []byte(contentType), []byte(jsonStr)}
	verifyFailure(t, stub, args, "Invoke should have failed to connect to google")

	// Failed Path: Http Status NOT OK
	args = [][]byte{[]byte("https://localhost:8443/test/statusNotOK"), []byte(contentType), []byte(jsonStr)}
	...
	args = [][]byte{[]byte("https://localhost:8443/test/xyz"), []byte(contentType), []byte(jsonStr)}
	if res := stub.MockInvoke("txID", args); res.Status == shim.OK {
...
func initHTTPServerConfig() {
	...
	viper.Set("http.tls.caCert.file", "./test-data/certs/server/test-client.crt")
	viper.Set("http.tls.cert.file", "./test-data/certs/server/server.crt")
	viper.Set("http.tls.key.file", "./test-data/certs/server/server.key")
