...
func testPeerGroupResolver(t *testing.T, sigPolicyEnv *common.SignaturePolicyEnvelope, peerRetriever PeerRetriever, expected []PeerGroup) {
	...
	level := logging.GetLevel(logger.Module)
	logging.SetLevel(logging.WARNING, logger.Module)
	defer logging.SetLevel(level, logger.Module)
...
func peer(name string) sdkApi.Peer {
	peer, err := sdkFabApi.NewPeer("localhost:7051", "", "", configImp)
	if err != nil {
