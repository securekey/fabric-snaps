...
type Client interface {
	...
	// @param {[]string} endorsersForCCIDs
...
func (c *clientImpl) EndorseTransaction(channel sdkApi.Channel, chaincodeID string,
	args []string, transientData map[string][]byte, targets []sdkApi.Peer, endorsersForCCIDs []string) (
	...
	if targets == nil {
		if len(endorsersForCCIDs) == 0 {
			endorsersForCCIDs = append(endorsersForCCIDs, chaincodeID)
		}
		// Select endorsers
