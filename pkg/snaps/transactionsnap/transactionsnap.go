...
import (
	...
	protos "github.com/securekey/fabric-snaps/pkg/snaps/transactionsnap/protos"
)
...
func endorseTransaction(stub shim.ChaincodeStubInterface) ([]*sdkApi.TransactionProposalResponse, error) {
	...
	var snapTxRequest protos.SnapTransactionRequest
	...
	channel, err := fcClient.NewChannel(snapTxRequest.GetChannelID())
	if err != nil {
		return nil, fmt.Errorf("Cannot create channel %v", err)
	}
	//cc code args
	endorserArgs := snapTxRequest.GetEndorserArgs()
	var ccargs []string
	...
	tpxResponse, err := fcClient.EndorseTransaction(channel, snapTxRequest.GetChaincodeID(),
		ccargs, snapTxRequest.GetTransientMap(), nil)
	if err != nil {
		return nil, err
	}
